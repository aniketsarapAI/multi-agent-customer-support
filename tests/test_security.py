...
class TestSecurityPipeline:
    ...
    def test_pipeline_allows_safe_input(self):
        is_allowed, cleaned, notes = self.pipeline.check_input("How many customers?")
    ...
    def test_pipeline_masks_input_pii(self):
        is_allowed, cleaned, notes = self.pipeline.check_input(
            "My email is john@test.com"
        )
        assert is_allowed is True
        assert "[EMAIL REDACTED]" in cleaned
        assert "PII" in " ".join(notes)

    def test_pipeline_validates_output_pii(self):
        safe, warnings = self.pipeline.check_output("Email me at a@b.com")
